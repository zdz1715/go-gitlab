...
func (ps *ProjectsService) ListProjects(ctx context.Context, req *ListProjectsOptions) ([]*Project, error) {
	const apiEndpoint = "/api/v4/projects"
	var p []*Project
	if err := ps.client.InvokeByCredential(ctx, http.MethodGet, apiEndpoint, req, &p); err != nil {
		return nil, err
	}
	return p, nil
}

// ProjectHookOptions represents the available add or edit options.
...
// GitLab API docs: https://docs.gitlab.com/ee/api/projects.html#add-project-hook
func (ps *ProjectsService) AddProjectHook(ctx context.Context, projectId string, req *ProjectHookOptions) (*ProjectHook, error) {
	apiEndpoint := fmt.Sprintf("/api/v4/projects/%s/hooks", projectId)
	var v *ProjectHook
	if err := ps.client.InvokeByCredential(ctx, http.MethodPost, apiEndpoint, req, &v); err != nil {
		return nil, err
	}
	return v, nil
}

// EditProjectHook edit a hook to a specified project.
// GitLab API docs: https://docs.gitlab.com/ee/api/projects.html#edit-project-hook
func (ps *ProjectsService) EditProjectHook(ctx context.Context, projectId string, hookId string, req *ProjectHookOptions) (*ProjectHook, error) {
	apiEndpoint := fmt.Sprintf("/api/v4/projects/%s/hooks/%s", projectId, hookId)
	var v *ProjectHook
	if err := ps.client.InvokeByCredential(ctx, http.MethodPut, apiEndpoint, req, &v); err != nil {
		return nil, err
	}
	return v, nil
}

func (ps *ProjectsService) DeleteProjectHook(ctx context.Context, projectId string, hookId string) error {
	apiEndpoint := fmt.Sprintf("/api/v4/projects/%s/hooks/%s", projectId, hookId)
	if err := ps.client.InvokeByCredential(ctx, http.MethodDelete, apiEndpoint, nil, nil); err != nil {
		return err
	}
	return nil
}
